...
const GROUPS: { label: string; items: { to: string; label: string; badge?: "scheduled" | "keep" }[] }[] = [
  { label: "OBSERVE", items: [
...
export function GlobalBanner() {
  const { data } = useQuery({ queryKey: ["healthz"], queryFn: () => endpoints.settings(), refetchInterval: 15000 });
  ...
  return (
    ...
    >
      ...
      <button className="font-normal underline opacity-80" onClick={() => navigate("/settings")}>
        Resume
      </button>
    </div>
  );
...
export function Shell({ children }: { children: ReactNode }) {
  ...
  const { data: dash } = useQuery({ queryKey: ["dash-badge"], queryFn: endpoints.dashboard, refetchInterval: 20000 });
  const { data: keeps } = useQuery({ queryKey: ["keep-badge"], queryFn: () => endpoints.keepRequests("pending") });
    ...
    <div className="flex min-h-screen flex-col">
      ...
      <div className="flex flex-1">
        <aside className="sticky top-0 flex h-screen w-[200px] flex-none flex-col gap-0.5 overflow-auto border-r border-line-subtle bg-bg-inset px-2.5 py-3">
          <div className="px-2 pb-3 pt-1 font-mono text-[13px] font-semibold text-ink-hi">▚ SWEEPARR</div>
          {GROUPS.map((g) => (
            <div key={g.label}>
