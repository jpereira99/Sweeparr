...
export function StatusPill({ state, size = "md", date, reason }: Props) {
  ...
  switch (state) {
    case "ACTIVE":
      cls = "bg-[rgba(139,150,168,0.12)] border border-[rgba(139,150,168,0.28)] text-ink-mid";
      break;
    case "SCHEDULED":
      cls = "bg-[rgba(229,72,77,0.14)] border border-[rgba(229,72,77,0.4)] text-state-scheduled-ink";
      label = date ? `SCHEDULED · ${fmtDate(date).toUpperCase()}` : "SCHEDULED";
      break;
    case "KEPT":
      cls = "bg-[rgba(63,162,111,0.13)] border border-[rgba(63,162,111,0.38)] text-state-kept-ink";
      label = reason ? `KEPT · ${reason.toUpperCase()}` : "KEPT";
      break;
    case "DELETING":
      cls = "bg-[rgba(107,116,135,0.1)] border border-[rgba(107,116,135,0.25)] text-state-muted";
      label = "DELETING…";
      break;
    case "DELETED":
      cls = "bg-[rgba(107,116,135,0.1)] border border-[rgba(107,116,135,0.25)] text-state-muted";
