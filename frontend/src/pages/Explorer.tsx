...
type SortKey = "title" | "type" | "last_watched" | "total_plays" | "distinct_watchers" | "completion" | "size" | "gb_per_hour";
...
const COLUMNS: {
  ...
  { id: "last_watched", label: "LAST WATCHED", sort: "last_watched", align: "right" },
  { id: "total_plays", label: "PLAYS", sort: "total_plays", align: "right" },
  { id: "distinct_watchers", label: "WHO", sort: "distinct_watchers", align: "right" },
...
export function Explorer() {
  ...
  function cycleTypeFilter() {
    setTypeFilter((t) => (t === "all" ? "movie" : t === "movie" ? "series" : "all"));
  }

  return (
    <div>
      <PageHeader title="Media Explorer" subtitle={`the whole library, sortable by "is it worth its disk?"`} />

      {isLoading ? (
        <Skeleton rows={8} />
      ) : items.length === 0 ? (
        <EmptyState title="No items match">
          {typeFilter !== "all" ? "Clear the type filter or try a different sort." : "Nothing in the library yet."}
        </EmptyState>
      ) : (
          ...
          <div className={`grid w-full ${cols}`}>
            ...
            {items.map((it: any) => (
              <div
                ...
                onKeyDown={(e) => e.key === "Enter" && setDrawerId(it.media_item_id)}
              ...
              >
                ...
                <span className="truncate text-[12.5px] font-medium text-ink-hi">
                  {it.title}
                  {it.unmanaged && <span className="ml-1 rounded bg-line px-1 text-[9px] text-ink-low">unmanaged</span>}
                </span>
                <span className="font-mono text-[11.5px] text-ink-mid">{relDays(it.last_watched_days)}</span>
                <span className="text-right font-mono text-[11.5px] text-ink-mid">{it.total_plays}</span>
                <span className="text-right font-mono text-[11.5px] text-ink-mid">{it.distinct_watchers}</span>
                <span className="text-right font-mono text-[11.5px] text-ink-mid">
                  {it.max_completion_pct ? `${Math.round(it.max_completion_pct)}%` : "—"}
                </span>
                <span className="text-right font-mono text-[11.5px] text-ink-hi">{gb(it.size_gb)}</span>
                <span
      ...
      {drawerId != null && <Drawer itemId={drawerId} onClose={() => setDrawerId(null)} />}
    </div>
  );
}
...
function ColumnHeader({
  ...
  if (col.filterType) {
    const label = typeFilter === "all" ? "All" : typeFilter === "movie" ? "MOV" : "TV";
    return (
      <button
        type="button"
      ...
      >
        <span className="text-[10.5px] font-semibold tracking-[0.08em]">TYPE</span>
  ...
  if (!col.sort) {
    return <span className={`text-[10.5px] font-semibold tracking-[0.08em] text-ink-low ${col.align === "right" ? "text-right" : ""}`}>{col.label}</span>;
  }
  ...
  return (
    ...
    >
      <span>{col.label}</span>
      {active && <span className="font-mono text-[9px]">{order === "desc" ? "↓" : "↑"}</span>}
    </button>
  );
}
