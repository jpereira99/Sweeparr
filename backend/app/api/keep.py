...
async def _serialize_kr(session: AsyncSession, kr: KeepRequest) -> dict:
    ...
    season_number = getattr(unit.obj, "season_number", None) if unit and kr.unit_type == "season" else None
    ...
    reason_public = _public_reason(getattr(unit.obj, "match_snapshot", None)) if unit else "Matched a removal rule"
    return {
        "id": kr.id, "unit_type": kr.unit_type, "unit_id": kr.unit_id, "title": title,
        "season_number": season_number, "requester": user.name if user else None,
        "reason": kr.reason, "status": kr.status, "created_at": _iso(kr.created_at),
        "delete_at": delete_at, "days_until": days_until, "token": kr.token,
        "size_gb": size_gb, "reason_public": reason_public,
    }


@router.get("/keep-requests")
async def list_keep_requests(status: str = "pending", session: AsyncSession = Depends(get_session), _: Principal = Depends(require_admin)):
...
async def my_keep_requests(session: AsyncSession = Depends(get_session), principal: Principal = Depends(current_principal)):
    stmt = select(KeepRequest).where(KeepRequest.user_id == principal.user_id).order_by(KeepRequest.created_at.desc())
    krs = (await session.execute(stmt)).scalars().all()
    return {"keep_requests": [await _serialize_kr(session, k) for k in krs]}


@router.post("/units/{unit_type}/{unit_id}/keep-request")
async def create_keep_request(unit_type: str, unit_id: int, body: KeepIn, session: AsyncSession = Depends(get_session), principal: Principal = Depends(current_principal)):
    ...
    existing = (
        await session.execute(
            select(KeepRequest).where(
                KeepRequest.unit_type == unit_type, KeepRequest.unit_id == unit_id, KeepRequest.status == "pending"
            )
        )
    ).scalars().first()
    if existing:
    ...
    kr = KeepRequest(
        unit_type=unit_type, unit_id=unit_id, user_id=principal.user_id,
        reason=body.reason, status="pending", token=secrets.token_urlsafe(12),
    )
    session.add(kr)
...
async def approve(kr_id: int, body: KeepDecision, session: AsyncSession = Depends(get_session), principal: Principal = Depends(require_admin)):
    ...
    unit = await lifecycle.get_unit(session, kr.unit_type, kr.unit_id)
    if unit:
        await lifecycle.keep_unit(session, unit, days=body.days, actor=principal.name, reason=f"keep approved: {kr.reason or ''}")
    await session.commit()
    return {"ok": True}


@router.post("/keep-requests/{kr_id}/deny")
async def deny(kr_id: int, body: KeepDecision, session: AsyncSession = Depends(get_session), principal: Principal = Depends(require_admin)):
    kr = await session.get(KeepRequest, kr_id)
    if not kr:
        raise HTTPException(404, "Not found")
    if not body.reason:
...
async def keep_by_token(token: str, session: AsyncSession = Depends(get_session)):
    ...
    kr = (await session.execute(select(KeepRequest).where(KeepRequest.token == token))).scalars().first()
    if kr:
