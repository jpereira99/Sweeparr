...
async def dashboard(
        ...
        gauges.append(
            {
                "root": root,
                "pct": pct,
                "used_tb": round(cap * pct / 100, 1),
                "capacity_tb": cap,
                "over_tier": over_tier,
                "warn_pct": tiers[0]["usage_pct"] if tiers else 85,
            }
        )

    scheduled = await _scheduled_units(
        session, [LifecycleState.SCHEDULED.value, LifecycleState.ERROR.value]
    )
    leaving = sorted(
        [u for u in scheduled if u["days_until"] is not None],
        key=lambda u: u["days_until"],
    )
    ...
    for d in deleted:
        ...
        cumulative.append(
            {
                "ts": d.ts.replace(tzinfo=timezone.utc).isoformat(),
                "cumulative_gb": round(running / GB, 1),
            }
        )
    ...
    jobs = (
        (
            await session.execute(
                select(JobRun).order_by(JobRun.started_at.desc()).limit(6)
            )
        )
        .scalars()
        .all()
    ...
    return {
        ...
        "scheduled_gb": round(
            sum(u["size_gb"] for u in scheduled if u["state"] == "SCHEDULED"), 1
        ),
        "disk_gauges": gauges,
...
async def stats_library(
    session: AsyncSession = Depends(get_session), _: Principal = Depends(require_admin)
):
    by_lib = (
        await session.execute(
            select(
                MediaItem.library, func.count(), func.sum(MediaItem.size_bytes)
            ).group_by(MediaItem.library)
        )
    ).all()
    return {
        "libraries": [
