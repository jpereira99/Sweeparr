...
@router.get("/flags")
async def flags(response: Response, jellyfin_ids: str = Query(""), session: AsyncSession = Depends(get_session)):
    ...
    movies = (
        await session.execute(
            select(MediaItem).where(
                MediaItem.jellyfin_id.in_(ids), MediaItem.state == LifecycleState.SCHEDULED.value
            )
        )
    ).scalars().all()
    for m in movies:
    ...
    rows = (
        await session.execute(
            ...
            .where(MediaItem.jellyfin_id.in_(ids), Season.state == LifecycleState.SCHEDULED.value)
        )
    ).all()
    for s, item in rows:
        items.append(_flag(item.jellyfin_id, s.delete_at, s.match_snapshot, season=s.season_number))
    return {"items": items}
...
def _flag(jf_id, delete_at, snapshot, season=None):
    da = None
    if delete_at:
        da = (delete_at if delete_at.tzinfo else delete_at.replace(tzinfo=timezone.utc)).date().isoformat()
    return {
        "jellyfin_id": jf_id,
