...
async def schedule(
    ...
    states = [LifecycleState.SCHEDULED.value, LifecycleState.DELETING.value, LifecycleState.ERROR.value]
    units = await _collect(session, states)
    units.sort(key=lambda u: (u["state"] != "ERROR", u["days_until"] if u["days_until"] is not None else 1e9))
    return {
        "units": units,
        "scheduled_count": len([u for u in units if u["state"] == "SCHEDULED"]),
        "total_gb": round(sum(u["size_gb"] for u in units if u["state"] == "SCHEDULED"), 1),
...
async def keep(unit_type: str, unit_id: int, body: KeepIn, session: AsyncSession = Depends(get_session), principal: Principal = Depends(current_principal)):
    unit = await _get_unit_or_404(session, unit_type, unit_id)
    days = body.days
    if not principal.is_admin:
        days = days or 30
    await lifecycle.keep_unit(session, unit, days=days, actor=principal.name, reason=body.reason)
    return {"ok": True, "state": unit.obj.state}


@router.post("/units/{unit_type}/{unit_id}/schedule")
async def schedule_unit(unit_type: str, unit_id: int, days: int = 30, session: AsyncSession = Depends(get_session), principal: Principal = Depends(require_admin)):
    unit = await _get_unit_or_404(session, unit_type, unit_id)
...
async def unschedule_unit(unit_type: str, unit_id: int, session: AsyncSession = Depends(get_session), principal: Principal = Depends(require_admin)):
    unit = await _get_unit_or_404(session, unit_type, unit_id)
...
async def postpone_unit(unit_type: str, unit_id: int, body: PostponeIn, session: AsyncSession = Depends(get_session), principal: Principal = Depends(require_admin)):
    unit = await _get_unit_or_404(session, unit_type, unit_id)
...
async def delete_now(unit_type: str, unit_id: int, session: AsyncSession = Depends(get_session), principal: Principal = Depends(require_admin)):
    unit = await _get_unit_or_404(session, unit_type, unit_id)
