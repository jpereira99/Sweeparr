...
@router.get("/jobs")
async def list_jobs(session: AsyncSession = Depends(get_session), _: Principal = Depends(require_admin)):
    states = scheduler.job_states()
    runs = (await session.execute(select(JobRun).order_by(JobRun.started_at.desc()).limit(30))).scalars().all()
    history = [{
        "job": r.job_name, "status": r.status, "summary": r.summary,
...
@router.get("/history")
async def history(action: Optional[str] = None, session: AsyncSession = Depends(get_session), _: Principal = Depends(require_admin)):
    stmt = select(AuditLog).order_by(AuditLog.ts.desc()).limit(200)
    if action:
        stmt = stmt.where(AuditLog.action == action)
    rows = (await session.execute(stmt)).scalars().all()
    entries = [{
        "ts": r.ts.replace(tzinfo=timezone.utc).isoformat(), "action": r.action, "actor": r.actor,
        "unit_type": r.unit_type, "unit_id": r.unit_id, "media_item_id": r.media_item_id, "detail": r.detail,
    } for r in rows]
    total_freed = sum((r.detail or {}).get("bytes_freed", 0) for r in rows if r.action == "deleted")
